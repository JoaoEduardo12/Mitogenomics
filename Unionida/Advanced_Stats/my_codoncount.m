function [outval,carray] = my_codoncount(dna,varargin)
...
%   NMERCOUNT, NTDENSITY, SEQRCOMPLEMENT, SEQSHOWORFS,
%   SEQWORDCOUNT.
...
if showFig
    ...
    for i = 1:8
        for j = 1:8
            if any(geneticCode~=0) && aal(i,j) == '*' 
               text(j,i,labels{i,j},'color',[0 0 0],...
    ...
    if any(geneticCode~=0)
      ...
      hold off 
      text(8.5,9,['Genetic Code: ' gc.Name],'color',[0 0 0],...
